...
async function experimentInit() {
  ...
  money_text = new visual.TextStim({
    ...
    font: 'microsoft yahei',
    units: undefined, 
    pos: [(- 0.2), 0], height: 0.05,  wrapWidth: undefined, ori: 0,
    languageStyle: 'LTR',
    color: new util.Color('white'),  opacity: 1,
    depth: -14.0 
  });
  ...
  stake_text = new visual.TextStim({
    ...
    font: 'microsoft yahei',
    units: undefined, 
    pos: [0.09, 0], height: 0.05,  wrapWidth: undefined, ori: 0,
    languageStyle: 'LTR',
    color: new util.Color('white'),  opacity: 1,
    depth: -20.0 
  });
...
function instrRoutineBegin(snapshot) {
  return async function () {
    ...
    token = 100;
    posl = [[(- 0.45), 0.3], [(- 0.35), 0.3], [(- 0.25), 0.3], [(- 0.15), 0.3], [(- 0.05), 0.3], [0.05, 0.3], [0.15, 0.3], [0.25, 0.3], [0.35, 0.3], [0.45, 0.3]];
...
function block_initRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of block_initComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function decision_trialRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of decision_trialComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function rate_trialRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of rate_trialComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
