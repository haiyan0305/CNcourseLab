...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='E:\\SCTC_LabVersion\\SCTC\\SocialCognitiveTestCollection\\Lab9_Cambridge_Gambling_Task\\Cambridge_Gambling_Task\\CGT_lastrun.py',
...
# --- Setup the Window ---
win = visual.Window(
    size=[1320, 640], fullscr=False, screen=0, 
...
introduction_text_2 = visual.TextStim(win=win, name='introduction_text_2',
    text='请准备好下注...\n按下向下方向键以停止下注',
    font='microsoft yahei',
    pos=(0, -0.03), height=0.04, wrapWidth=None, ori=0.0, 
    color='white', colorSpace='rgb', opacity=None, 
...
for thisBlock in blocks:
    ...
    for thisTrial in trials:
        ...
        # keep track of which components have finished
        rate_trialComponents = [choice_stake, image_9_0, image_10, image_11, image_12, image_13, image_14, image_15, image_16, image_17, image_18, red_image_2, blue_image_2, money_text_2, count_down_text_0, count_down_text_1, count_down_text_2, count_down_text_3, count_down_text_4, stake_text, arrow_image_2, introduction_text_2]
        ...
        while continueRoutine:
            ...
            if introduction_text_2.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                # keep track of start time/frame for later
                introduction_text_2.frameNStart = frameN  # exact frame index
                introduction_text_2.tStart = t  # local t and not account for scr refresh
                introduction_text_2.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(introduction_text_2, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'introduction_text_2.started')
                introduction_text_2.setAutoDraw(True)
            if introduction_text_2.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > introduction_text_2.tStartRefresh + ct5-frameTolerance:
                    # keep track of stop time/frame for later
                    introduction_text_2.tStop = t  # not accounting for scr refresh
                    introduction_text_2.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'introduction_text_2.stopped')
